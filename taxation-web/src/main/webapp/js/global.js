...
function initController($scope, thisobj, thisobjName, thisobj_service, filterFilter) {
    ...
    thisobj.searchN = function search() {//查询第N页
        ...
        pageData.then(function (data) {  // 调用承诺API获取数据 .resolve
            ...
            if(!isNull(data.dictType)){
                thisobj.dictType = data.dictType;
            }
...
function timeFormatTwoTo(time){
    var date = time.substring(0,4)+"-"+time.substring(4,6)+"-"+time.substring(6,8)+" "+time.substring(8,10)+":"+time.substring(10,12)+":"+time.substring(12,14);
    return date;
}

function timeFormatOneTo(time){
